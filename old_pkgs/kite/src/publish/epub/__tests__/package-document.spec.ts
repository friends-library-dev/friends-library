...
describe('packageDocument()', () => {
  ...
  it('contains the file-as meta tag', () => {
    ...
    expect(xml).toContain(
      '<meta property="file-as" refines="#author">Fox, George</meta>',
    );
  });
  ...
  it('must contain dc:terms modified', () => {
    ...
    const xml = packageDocument(job);
    expect(xml).toContain(
      '<meta property="dcterms:modified">2018-07-24T08:43:43Z</meta>',
    );
  });
  ...
  test('uuid should be constructed to uniquely refer to ebook doc', () => {
    const xml = packageDocument(job);
    expect(xml).toContain(
      `<dc:identifier id="pub-id">friends-library/${job.id}</dc:identifier>`,
    );
  });
...
describe('manifestItems()', () => {
  ...
  it('always includes css item', () => {
    ...
    expect(items.get('css')).toMatchObject({
      href: 'style.css',
      'media-type': 'text/css',
    });
  });
