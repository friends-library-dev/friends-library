...
<nav epub:type="toc" id="toc">
  ...
  <ol class="table-of-contents">
    ...
    ${sections.filter(s => s.id !== 'notes').map(sect => `<li><a href="${sect.id}.xhtml">${entryText(sect, config)}</a></li>`).join('\n      ')}
  </ol>
</nav>
...
<section class="content-toc">
  ...
  ${sections.filter(s => s.id !== 'notes').map(sect => `<div>${M7BR}<a href="${sect.id}.xhtml">${entryText(sect, config)}</a>${M7BR}</div>`).join('\n      ')}
</section>
  `.trim());
}
