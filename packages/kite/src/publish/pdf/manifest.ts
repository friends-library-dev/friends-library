...
export function getPdfManifest(job: Job): FileManifest {
  return {
    'doc.html': pdfHtml(job).replace(
      '</head>',
      '<link href="doc.css" rel="stylesheet" type="text/css"></head>',
    ),
    'doc.css': getCss(job),
...
function getSassVars(job: Job): { [key: string]: string } {
  const {
    target,
    spec: { meta, sections, config },
  } = job;
  const title =
...
export function printDims(job: Job): { [key: string]: string } {
