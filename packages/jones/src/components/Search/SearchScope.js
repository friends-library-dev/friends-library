...
import { searchedFiles, currentTask, currentTaskFriendName } from '../../select';
...
class Component extends React.Component<Props> {
  ...
  renderEdition() {
    ...
    return (
      <>
        * Searching <code>{numFiles}</code> files in
        the <i className="edition">{editionType}</i> edition
        of <i>{documentTitle}</i>.
      </>
    );
  }
...
const mapState = state => {
  ...
  return {
    ...
    documentTitle: documentSlug ? task.documentTitles[documentSlug] : null,
