...
export default () => (
  <Section id="footnotes-section">
    <H1>Footnotes</H1>
    <Note>
      Browsers can't render our footnotes, so all of the examples
      in this section are only shown in asciidoc. You'll need
      to created PDFs using <code>kite</code> to actually see
      rendered footnotes. Sorry! 😬
    </Note>
    <Para>
      Footnotes in asciidoc are created with the <i>footnote macro</i>, which
      looks like <Code>footnote:[Some text here]</Code>, as shown below:
    </Para>
    <Asciidoc id="footnote-inline" />

    <Para>
      However, if there's not punctuation before a footnote, the macro
      runs directly into the text, which looks weird, so I created a convention
      that we can also add a caret <Code>^</Code> and then a newline
    ...
    <Para>
      ...
      can incorporate inline styling within their content, including things
      like <b>italics</b> and <b>inline quotes</b>:
    </Para>
