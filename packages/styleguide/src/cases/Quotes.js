...
export default () => (
  <Section id="quotes">
    ...
    <ul>
      <li><Code>"`</Code> is an <i>open smart quote</i>.</li>
      <li><Code>`"</Code> is an <i>close smart quote</i>.</li>
      <li><Code>'`</Code> is an <i>open smart apostrophe</i>.</li>
      <li><Code>`'</Code> is an <i>close smart apostrophe</i>.</li>
    </ul>
    ...
    <Note>
      As you working with documents, <b>don't fix straight apostrophes</b>. We
      should be able to programatically fix all these at one time, so it's not
    ...
    <Para>
      Block Quotes can also specify <b>attribution</b> by listing
      first the source, then the location, comma-separated, in the opening
      line, like this <Code>[quote, AUTHOR, LOCATION]</Code>.
      The author and location will be rendered <i>below</i> the quote,
      as shown below:
    </Para>
    <Pair id="quote-cited" emphasize={[1]} />
    <Note>
      As shown in the above example, if the <i>author</i>, or <i>location</i> contains
      one or more <b>commas</b>, you'll need to wrap it with quotation
      marks. <Code>[quote, "G. Fox, M.D.", "Book, etc."]</Code>
    </Note>
    <Para>
      If you <b>only have a location</b>, you can skip the <i>author</i> by
      leaving it blank:
    </Para>
    ...
    <Para>
      For blockquotes that are from <b>scripture</b>, add the
      class <Code>.scripture</Code>, and an <i>optional</i> citation.
